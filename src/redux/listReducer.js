    ...
    switch (action.type) {
        case sort:
            if (action.sortType == 1){
                return {...state, elements: [...state.elements.sort((a, b) => {
                    if (a.name.localeCompare(b.name) == 0) {
                        return b.count - a.count
                    }
                    else {
            ...
            else if (action.sortType == 2){
                return {...state, elements: [...state.elements.sort((a, b) => a.id - b.id)]}
            }
            else if (action.sortType == 3){
                return {...state, elements: [...state.elements.sort((a, b) => {
                    if (a.name.localeCompare(b.name) == 0) {
                        return b.count - a.count
                    }
        ...
        case addNewElement:
            let allIds = [state.elements.map(i => i.id)]
            console.log(allIds[0])
            let id = Math.max(...allIds[0]) + 1
            let warn = false
            ...
            let element = model(id, state.newElement.imageUrl, state.newElement.title,
...
export let sortAction = (sortType) => ({type: sort, sortType})
